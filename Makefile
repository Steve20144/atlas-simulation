...
menu:
	$(UV) python scripts/vectra_menu.py

# Backend on :8000 (uvicorn --reload) and Vite dev server on :5173, together.
dev:
	set -euo pipefail
	$(UV) uvicorn vectra.api.app:app --reload --port 8000 &
	backend_pid=$$!
...
guide:
	cd docs/guide && latexmk -pdf -interaction=nonstopmode -outdir=build gazebo_px4_guide.tex && cp build/gazebo_px4_guide.pdf gazebo_px4_guide.pdf
