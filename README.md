...
**Hover attitude as a design parameter.** `frame.hover_pitch_deg` (nose-up degrees) is the attitude the airframe holds in hover; PX4's body frame, where the `CA_ROTOR*` geometry lives, is that hover frame, so every effective force point and thrust axis is rotated about +Y by it before allocation (`Scenario.hover_pos` / `hover_axis`), and the Gazebo and HITL models carry the CAD mesh rotated the same way. Every parameter set the app produces (params preview, `.params` export, HITL package, upload to the board) also writes `SENS_BOARD_Y_OFF` = `hover_pitch_deg`, the PX4 board-to-body pitch offset (`src/modules/sensors/sensor_params.c`, applied in `src/lib/sensor_calibration/Accelerometer.cpp:142` as `Dcm(Euler(X_OFF, Y_OFF, Z_OFF)) * board_rotation`), so the flight controller holds the nose-up hover attitude as level. It assumes the board is mounted aligned with the airframe FRD axes (`SENS_BOARD_ROT` covers a rotated mount) and it overwrites a level-horizon calibration; PX4 limits the parameter to -45 to 45 degrees, so previews and exports refuse a hover pitch beyond that. Hovering nose-up by t is equivalent, for the wing jets, to a level hover with each foil deflected t degrees further. The sweep takes a list of attitudes (`hover pitch°` in the panel, `--hover-pitch=-30:30:15` on the shell) and evaluates every geometry at each; rows show `@+15` for a pitched hover. On the CAD layout the as-built 45 degree foils trim at no attitude (wing and nose fans always disagree in direction), but pitched hovers open cheaper trims: 135/45/90/45 at +15 degrees nose-up hovers at 11.0 kW with roll 21.7, pitch 10.9 and yaw 7.1 rad/s^2, against 12.5 kW for the best level set with the same yaw requirement. Attitudes beyond about 15 degrees cost roll authority and headroom quickly.
...
### Upload parameters to the Pixhawk from the app

The **board** pill in the top bar (and the chip icon at the bottom of the left rail) asks the backend whether a Pixhawk 6X Pro is heartbeating on USB: the backend scans the serial ports for the fmu-v6x USB id (Holybro 0x3185:0x0035 in the pinned tree's `boards/px4/fmu-v6x/nuttx-config/nsh/defconfig`) or a PX4-looking name, opens the port, waits for the autopilot heartbeat and reads `AUTOPILOT_VERSION`. The **Pixhawk 6X Pro** card in the right column has the same check with a port selector, and **Upload params**, which needs a second confirming click, writes exactly the lines shown in the params preview to the board. Writes go through the NSH shell (`param set NAME VALUE` over `SERIAL_CONTROL`, then `param save`) because PX4 carries INT32 parameters byte-wise in `PARAM_VALUE` and a plain float `PARAM_SET` corrupts every integer parameter; before writing, the backend reads the board's current values of those parameters into `exports/board/<timestamp>_<scenario>_before.params`, and after writing it reads every value back and lists mismatches. The backend must run natively on the machine that has the USB cable (not in Docker), the fans and ESCs must be unpowered, and a running HITL session owns the port, so stop it first (the endpoints answer 409). The CLI equivalent is `scripts/px4_board.py`. **Latest flight data**, next to Check, downloads the newest `.ulg` the board recorded (MAVLink `LOG_REQUEST_LIST` / `LOG_REQUEST_DATA`; newest by GPS time, then by id; stalled chunks are re-requested) into `exports/logs/` and offers it as a download (`POST /api/board/pull_log`, `GET /api/board/logs/<file>`). USB moves about 50 kB/s, so a long flight takes minutes; QGC must not hold the port meanwhile. **Test thrust** opens a live view of the thrust path while the board runs (`POST /api/board/thrust_feed/start`, polled at 5 Hz, `backend/vectra/px4/telemetry.py`): the raw throttle pulse (`RC_CHANNELS`), PX4's throttle after calibration and dead zone (`MANUAL_CONTROL`, turned on at 10 Hz with `mavlink stream`), the collective the attitude controller asks for (`ATTITUDE_TARGET.thrust`), the pulse width on every MAIN and AUX output (`SERVO_OUTPUT_RAW` instances 0 and 1), the armed flag, a 30 s trace and the parameters that shape the curve (`MPC_THR_HOVER`, `MPC_MANTHR_MIN`, `THR_MDL_FAC`, `MC_AIRMODE`, `RC3_*`). The feed holds the board lock, so Check and Upload wait until Stop. It is a measurement, not a lockdown: PX4 disarms 5 s after a software lockdown unless it boots in HITL (`Commander.cpp:2314-2329`), so the outputs stay live and the fans and ESCs must be unpowered while you arm and move the stick.

Once a check has succeeded the card also shows **SYS_HITL** as stored on the board with **HIL on** / **HIL off** buttons and **Reboot board**. **HIL on** writes `SYS_HITL 1` (`POST /api/board/param`, the type the board reports, saved and read back). **HIL off** does more, because a bare `SYS_HITL 0` does not survive a reboot: the HITL parameter set puts `SYS_AUTOSTART` on PX4's HIL airframe 1001, and that airframe script runs `param set SYS_HITL 1` at every boot (`ROMFS/px4fmu_common/init.d/airframes/1001_rc_quad_x.hil:14`), while rcS also zeroes `GPS_1_CONFIG` in HIL mode (`rcS:329`). So **HIL off** (`POST /api/board/flight`, `backend/vectra/px4/flight.py`) writes `SYS_HITL 0` together with `SYS_AUTOSTART`, `EKF2_EN`, `SYS_HAS_MAG`, `SYS_HAS_BARO`, `CBRK_SUPPLY_CHK`, `GPS_1_CONFIG`, the IMU calibration slots the HITL set overwrote and the HITL controller gains, all taken from the newest flight `.params` backup (the same file the export merges onto; PX4 defaults where the backup lacks a line, with a warning in the message), backs up the board's current values to `exports/board/`, saves and reads back. The message lists what was restored and any warnings; reboot afterwards and re-check that `SYS_HITL` reads 0. **Reboot board** (`POST /api/board/reboot`, `MAV_CMD_PREFLIGHT_REBOOT_SHUTDOWN`, second click confirms; the app re-checks the board 8 s later). PX4 reads `SYS_HITL` at boot only (`rcS` line 324 of the pinned tree starts the sensors in HIL mode when it is > 0, line 332 picks SIH when it is 2), so the toggle is not live until the reboot; HITL on the 6X also needs the custom firmware build with `pwm_out_sim`, see the HITL section above.

Fan angles in the geometry tables are entered as **fwd / side** by default: forward tilt leans the thrust forward (+) or aft (-) and side tilt leans it right (+) or left (-), each measured from vertical in its own plane, which is how a mount is drawn and printed, and the two are independent (a vertical nose fan leaned 25 degrees to the right is fwd 0, side 25). The **tilt / azimuth** tab shows PX4's convention instead (one tilt from vertical plus the compass direction it leans towards, 0 forward, 90 right); both edit the same `tilt_deg` / `azimuth_deg` fields (axis is proportional to (tan fwd, tan side, -1)), so the exported `CA_ROTORn_A*` do not depend on the mode. In the tilt / azimuth view the tilt accepts -180 to 180: a negative tilt is the same lean the opposite way (the azimuth turns by 180, so 30 and -30 on the two nose fans give one leaning forward and one aft), and values past 90 point the thrust below the horizon; fwd / side entry stops at the horizon and shows a note for such a fan.
...
| `POST /api/px4_params_preview` `{scenario, concept?}` | CA_* parameter lines plus `SENS_BOARD_Y_OFF` = hover pitch (400 when the pitch exceeds 45 degrees) |
...
- Before writing an exported `.params` to the vehicle, diff it against your backup: only `CA_ROTOR*`, `SENS_BOARD_Y_OFF` (the hover pitch), and for the fully-actuated concept `CA_METHOD` and `FD_FAIL_*`, should change.
- `SENS_BOARD_Y_OFF` replaces the board's level-horizon calibration with the hover pitch. If the board is not mounted flush with the airframe, set `SENS_BOARD_ROT` for the mount first; the app never touches `SENS_BOARD_X_OFF` or `SENS_BOARD_Z_OFF`.
