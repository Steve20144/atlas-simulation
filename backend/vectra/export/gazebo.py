...
AIRFRAME_ID_BASE = 4500  # vectra ids are 4500..4899; PX4 v1.17 uses none of them


def airframe_id(name: str) -> int:
    """Autostart id for a scenario name: PX4's rcS matches `<id>_*` and sources the last hit, so two
    vectra exports installed side by side must not share an id."""
...
def px4_tuning(scenario: Scenario) -> dict[str, float]:
    """PX4 controller parameters sized for this airframe from vectra's own numbers.
    ...
    limit 0.15, attitude P = w_c / 2.5, autotune off. Authority is vectra's attainable torque
    ...
    collective. Axes vectra marks unattainable keep the PX4 defaults.
    """
...
def px4_airframe(scenario: Scenario, name: str) -> str:
    ...
    lines = [
        "#!/bin/sh",
        f"# @name vectra {name} (gz)",
        "# @type Multirotor",
        "# @class Copter",
        "# Generated by vectra from the scenario; CA_ROTOR* match the exported .params.",
    ...
    lines += [
        "",
        "# Controller sizing from vectra (see px4_tuning in vectra/export/gazebo.py): rate-loop",
        ...
        *(f"param set-default {k} {v:g}" for k, v in tuning.items()),
        ...
        '\tif [ -n "${gcs_host}" ]',
        "\tthen",
        '\t\techo "INFO  [init] vectra: MAVLink GCS link to ${gcs_host}:${gcs_port} (WSL2 host)"',
...
Generated by vectra; not run on the machine that generated it (Gazebo needs a Linux host with
PX4 v1.17 and gz sim Harmonic). Frames: gz body is FLU; vectra's FRD (x, y, z) maps to (x, -y, -z).
...
meant to fly (the airframe file carries the same list in its header). A set that vectra reports
...
1. QGroundControl must be connected (PX4 refuses to arm with "No connection to the GCS"). PX4 SITL
   ...
   `PX4_GCS_PORT` override the target; the console prints `vectra: MAVLink GCS link to ...`.
...
vectra writes LF; if you edit or copy the files on Windows, run `sed -i 's/\\r$//'` on them in WSL.
...
2. In Stabilized on the ground, raising the throttle spins rotors 8 and 9 (centreline) and the
   wing rotors according to the allocator; compare with vectra's hover `u`.
3. Hover in Position mode. If the vehicle drifts fore-aft at level attitude, the geometry has a
   net Fx, which vectra reports as "no level-attitude hover trim".
...
MPC_THR_HOVER at vectra's hover collective. Axes vectra marks unattainable keep PX4 defaults.
...
def export_gazebo(
    ...
    (model_dir / "model.config").write_text(
        ...
        f'  <sdf version="1.9">model.sdf</sdf>\n  <author><name>vectra</name></author>\n'
        f"  <description>{escape(scenario.meta.name)} generated by vectra</description>\n"
        "</model>\n",
        encoding="utf-8",
    )
