...
def model_sdf(scenario: Scenario, name: str, mesh_uri: str | None, serial: str) -> str:
    ...
    out = [
        f"<!-- Generated by vectra from scenario {escape(scenario.meta.name)} for HITL "
...
(MAVLink HIL_SENSOR / HIL_GPS in, HIL_ACTUATOR_CONTROLS out). Generated by vectra, not run here.
...
in this geometry, which vectra reports as "no level-attitude hover trim".
...
def export_gazebo_classic_hitl(
    ...
    (model_dir / "model.config").write_text(
        ...
        f"  <description>{escape(scenario.meta.name)} HITL model generated by vectra"
        "</description>\n"
        "</model>\n",
        encoding="utf-8",
    )
    ...
    pf = params_file_from_dict(
        ...
        header=default_header(
            note=f"vectra HITL set for {scenario.meta.name}: HIL airframe, SYS_HITL 1, "
