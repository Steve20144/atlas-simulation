#!/usr/bin/env bash
# px4ctl.sh: talk to the PX4 SITL instance that vectra_gazebo.sh started (same WSL distro).
#
...
#   bash px4ctl.sh log [<windows dir>]       # copy the newest .ulg out (default: the vectra exports dir)
...
# (vectra/export/gazebo.py px4_tuning) or the airframe file; PX4 persists them in
# build/px4_sitl_default/rootfs/parameters.bson, which vectra_gazebo.sh offers to clear.
set -euo pipefail
...
case "$cmd" in
  ...
  reset) bash "$(dirname "$0")/vectra_gazebo.sh" --reset --mode sitl ;;
