...
- HITL exit: airframe `ROMFS/px4fmu_common/init.d/airframes/1001_rc_quad_x.hil:14` runs `param set SYS_HITL 1` every boot, rcS:324-329 starts sensors in HIL mode and zeroes GPS_1_CONFIG, rcS:466-470 starts pwm_out_sim. A bare SYS_HITL 0 therefore comes back as 1 while SYS_AUTOSTART is 1001; `px4/flight.py flight_params` restores the whole HITL set from the flight backup (`POST /api/board/flight`, the app's HIL off).
- Board level offsets: `src/modules/sensors/sensor_params.c` (SENS_BOARD_ROT line 168, SENS_BOARD_Y_OFF lines 170 to 183, range -45 to 45), applied as `Dcm(Euler(X_OFF, Y_OFF, Z_OFF)) * board_rotation` in `src/lib/sensor_calibration/Accelerometer.cpp:142` and `Utilities.cpp:207-217`; Euler to Dcm convention `src/lib/matrix/matrix/Dcm.hpp:121-142`. The app writes SENS_BOARD_Y_OFF = frame.hover_pitch_deg (`core/params_px4.py hover_frame_params`).

## Environment notes (this development machine, Windows 11)
- Tools are installed in user space and are NOT on PATH in shells spawned by the running Codex app. In Git Bash run `source scripts/env.sh` first; in PowerShell `. .\scripts\env.ps1`. Locations: uv and make in `~/.local/bin`, node and npm in `%LOCALAPPDATA%\Programs\nodejs`.
